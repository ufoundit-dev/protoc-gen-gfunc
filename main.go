...
func generateFile(gen *protogen.Plugin, file *protogen.File) (*protogen.GeneratedFile, error) {
	...
	fmt.Fprintln(os.Stderr, "=====GoImportPath", file.GoImportPath)
	...
	if nil != opts {
		fmt.Fprintln(os.Stderr, "=====opts", opts)
		fileOpts := proto.GetExtension(opts, gfunc.E_GfuncFile).(*gfunc.GfuncFileOptions)
		if fileOpts != nil {
			fmt.Fprintln(os.Stderr, "=====fileOpts", fileOpts)

			for _, i := range fileOpts.Imports {
				g.P("import " + i)
			}
		}
	}
	...
	for _, msg := range file.Messages {
		...
		vv := proto.GetExtension(opts, gfunc.E_GfuncMessage)
		if vv == nil {
			continue
		}
