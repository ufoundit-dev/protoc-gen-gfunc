...
func generateFile(gen *protogen.Plugin, file *protogen.File) (*protogen.GeneratedFile, error) {
	...
	if nil != opts {
		vv := proto.GetExtension(opts, gfunc.E_GfuncFile)
		if vv != nil {
			gfuncFileOptions := vv.(*gfunc.GfuncFileOptions)
			for _, i := range gfuncFileOptions.Imports {
				g.Import(protogen.GoImportPath(i))
			}
		}
	}
